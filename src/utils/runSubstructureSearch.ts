...
export function runSubstructureSearch(
  ...
  // The search takes batches, so candidates are buffered as the prescreen yields them and tested
  // a batch at a time. Matches come back in prescreen order, so the lightest-first guarantee — and
  ...
  // An empty fragment is contained in every molecule, so nothing is parsed for it.
  const flush = () => {
